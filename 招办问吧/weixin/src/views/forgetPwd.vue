  ...
  <div class="register">
    ...
    <div class="form">
      <div id="input">
        ...
        <hr>
        <div class="list">
          <input type="password" placeholder="请输入您的新密码" v-model="password" @click="changeHeight($event)">
          <img :src="emptySrc" class="empty" @click="empty('password')" v-if="password.length>0">
        </div>
        <hr>
        <div class="list">
          <input type="password" placeholder="请再次输入您的新密码" v-model="password2" @click="changeHeight($event)">
      ...
      <!-- 找回密码 -->
      <a href="javascript:void(0);" class="regBtn retrieve" @click="retrieve" @touchstart="hover($event, true)" @touchend="hover($event, false)">确&nbsp;&nbsp;定</a>
    </div>
