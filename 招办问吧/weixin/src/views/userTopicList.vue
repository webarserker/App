<template>
    <div class="box box-ver h-p100" id="wrap">
        <nav-path>
            ...
            &gt;&nbsp;
            <div class="ellipsis box-f1">我的坐席</div>
        </nav-path>
...
export default {
  ...
  methods: {
    ...
    getTopicList () {
      ...
      var state = self.$store.state
      self.isRequest = true
      var url = state.host + state.baseUrl + '/topic/findTopicList?userId=' + state.userId + '&curPage=' + self.curPage + '&pageSize=5'
      this.$http.get(url).then(res => {
        self.isRequest = false
        var data = res.data.data.data
        if (!data || data.length === 0) {
          if (self.curPage === 1) {
            Toast({message: '您暂未创建任何话题', duration: 1000})
            self.topicList = []
          } else {
            self.showEnd = true
          }
