<template>
    <div>
        <ul class="section-links">
            ...
            <!--<p class="underline" :style="{backgroundColor:$store.state.themeColor}"></p>-->
            <!--</li>-->
            <li @click="sliderX(scrollList.length)" :style="{color:$store.state.themeColor}" :class="{'link-active':currentPageIndex==scrollList.length}">
                视频在线
                <p class="underline" :style="{backgroundColor:$store.state.themeColor}"></p>
            </li>
        </ul>
...
//                    }
        ...
        updated(){
            if(this.scrollList.length>0 && this.sliderInit==0){
                setTimeout(() => {
                    this.setSliderWidth()
                    this.initSlider()
                    this.sliderInit++;
