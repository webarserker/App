<template>
    <div>
        ...
        <div class="home-container v2-container">
            ...
            <div class="v2-sections" v-show="modules.length<=4&&modules.length>=2">
                <div :class="['num'+(modules.length+1),'div0']" @click="gotoOnline()">
                    <div class="one">
                        <div class="icon-container">
                            <i class="usericon-webicon-48"></i>
                        </div>
                        <div class="right">
                            <h2>智能答疑</h2>
                        </div>
                ...
                <div v-for="(item,index) in modules" :class="['num'+(modules.length+1),'div'+(index+1)]"
                     v-if="modules.length==2" @click="jump(item.ntype,item.catId,item.refUrl)">
                    <div :class="{'two':index==0,'three':index==1}">
                        <div class="icon-container">
                ...
                <div v-for="(item,index) in modules" :class="['num'+(modules.length+1),'div'+(index+1)]"
                     v-if="modules.length==3" @click="jump(item.ntype,item.catId,item.refUrl)">
                    <div :class="{'two':index==0,'three':index==1,'four':index==2}">
                        <div class="icon-container">
                            <i :class="item.icon?item.icon:'usericon-webicon-01'"></i>
                        </div>
                        <div class="right">
                ...
                <div v-for="(item,index) in modules" :class="['num'+(modules.length+1),'div'+(index+1)]"
                     v-if="modules.length==4" @click="jump(item.ntype,item.catId,item.refUrl)">
                    <div :class="{'two':index==0,'three':index==1,'four':index==2,'five':index==3}">
                        <div class="icon-container">
                            <i :class="item.icon?item.icon:'usericon-webicon-01'"></i>
                        </div>
                        <div class="right">
            ...
            <div class="v2-more-sections" ref="scrollWrapper">
                <ul ref="sectionWrapper">
                    <li v-for="(item,index) in baokao" :class="'bg'+index"
                        @click="goToList(item.refUrl,item.catId)">
                        <div>
                            <i :class="item.icon?item.icon:'usericon-webicon-01'"></i>
            ...
            <div id="slider-wrapper">
                <Slider :scrollList="scrollList">
                    <div v-for="(item,index) in scrollList" class="scroll-list-item slider-item">
                        <ul>
                            <li v-for="(subItem,subIndex) in item.children?item.children.slice(0,3):[]"
                                @click="anyJump(subItem.ntype,subItem.refUrl,subItem.id,item.ntype,item.catId,subItem.catId)">
                                <div class="img-container" v-if="subItem.img">
                                    <img :src="$store.state.imgHost+subItem.img" >
                                </div>
                                <div :class="['text-container',{'noImg':!subItem.img}]">
                                    <i class="icon-lingdang-v2" v-if="!subItem.isRead"></i>
                                    <div class="title">
                                        {{subItem.name}}
                                    </div>

                                    <div class="release-time">
                                        <span :style="{backgroundColor:$store.state.themeColor}" v-text="getYear(subItem.date)"></span>
                                        <span :style="{backgroundColor:$store.state.themeColor}"
                                              v-text="getMonthAndDay(subItem.date)"></span>
                                    </div>
                                </div>
                            </li>
                        </ul>
                        <div class="see-more" @click="item.ntype==9?$router.push('/noticeList?name='+item.name):item.ntype==10?$router.push('/newsList?name='+item.name):$router.push('/articleList?catId='+item.catId)">
                            查看更多
                        </div>
                    </div>
                    ...
                    <!--@click="$router.push('/articleDetail?infoId='+item.infoId+'&catId='+newsCatId)">-->
                    <!--<div class="img-container" v-if="item.img">-->
                    <!--<img :src="$store.state.imgHost+item.img" >-->
                    <!--</div>-->
                    <!--<div :class="['text-container',{'noImg':!item.img}]">-->
                    <!--<i class="icon-lingdang-v3new" v-if="!item.isRead"></i>-->
                    <!--<div class="title">-->
                    <!--{{item.name}}-->
                    <!--</div>-->

                    <!--<div class="release-time">-->
                    <!--<span :style="{backgroundColor:$store.state.themeColor}" v-text="getYear(item.date)"></span>-->
                    <!--<span :style="{backgroundColor:$store.state.themeColor}"-->
                    ...
                    <!--<ul>-->
                    <!--<li v-for="(item,index) in noticeList" @click="$router.push('/articleDetail?infoId='+item.infoId+'&catId='+item.catId)">-->
                    <!--<div class="img-container" v-if="item.img">-->
                    <!--<img :src="$store.state.imgHost+item.img">-->
                    <!--</div>-->
                    <!--<div :class="['text-container',{'noImg':!item.img}]">-->
                    ...
                    <!--</li>-->
                    <!--</ul>-->
                    <!--<div class="see-more" @click="$router.push('/noticeList')">-->
                    <!--查看更多-->
                    <!--</div>-->
                    <!--</div>-->
                    <div class="vedios">
                        <ul>
                            <li v-for="(item,index) in video">
                                <div class="vedio-container" @click="bannerClick(item.vidoUrl)">
                                    <div class="bg-img":style="{'background':'url('+$store.state.imgHost+item.imgUrl+') 0 0 no-repeat','background-size':'100% 100%'}">
...
//                    location.href = state.talkHost + '/users?id=' + user.id + '&sCode='+state.scode+'&name='+user.nickName+'&professor='+state.scholl.professor+'&school=' + state.scholl.name
            ...
            anyJump(subItemNtype,subItemRefUrl,subItemId,itemNtype,itemCatId,subItemCatId){
                subItemNtype==2?window.location.href=subItemRefUrl:this.$router.push('/articleDetail?infoId='+subItemId+'&catId='+(itemNtype==0?itemCatId:subItemCatId))
            },
...
//                })
            ...
            this.$http.get(state.host+'/front/template/'+state.scode+'/'+state.mVersion+'/UserDefined').then(res=>{
                res=res.body;
                this.modules=res.data?res.data.slice(0,4):[];
            })
            ...
            this.$http.get(state.host+'/front/template/'+state.scode+'/'+state.mVersion+'/TabControl').then(res=>{
                res=res.body;
                console.log('选项卡\n',res)
                if(res.data==null){
                    this.scrollList=[];
                }else if(res.data.length>4){
                    this.scrollList=res.data.slice(0,4);
                }else{
