            ...
            <div class="article-body" ref="articleBody">
                ...
                <div class="title-container" ref="titleContainer">
                    ...
                    <div class="info" v-if="infoId && catId">
                        <p class="date" v-if="infoId && catId">
                            发布时间：{{publishTime}}
                        </p>
                        <p class="see" v-if="infoId && catId">
                            浏览量：{{viewTimes ? viewTimes : majorViewTimes}}
                        </p>
                    </div>
                </div>
                ...
                <v-comp-0 v-if="specType==0" :infoId="infoId" :tid="$route.query.tid" class="comp"
                          @getViewTimes="getViewTimes"></v-comp-0>

                <v-comp-school-detail @changeSpecType="changeSpecType" v-if="specType=='schoolDetail'"
                                      :sId="$route.query.sId" class="comp"></v-comp-school-detail>


                <v-comp-research-detail v-if="specType=='research-detail'" :id="$route.query.id"
                                        class="comp"></v-comp-research-detail>
                <!-- <v-table1 ></v-table1> -->
                ...
                <div class="dcode_area">
                    ...
                    <div class="dcode_item">
                        <img v-if="$store.state.allSchoolInfo"
                             :src="$store.state.imgHost+$store.state.allSchoolInfo.wxSchoolOfficeQrcode"
                             title="qrcode" alt="qrcode" @click="$store.state.sdecode=true"/>
...
//                return this.content || document.getElementsByClassName('comp').length
        ...
        components: {
            ...
            "v-comp-0": require('../component/comp-0.vue'),
            "v-comp-research-detail": require('../component/comp-research-detail.vue'),
            ...
            /*****************新注册一个院系详情页组件************************/
            "v-comp-school-detail": require('../component/comp-school-detail.vue')
            /*****************************************/
        ...
        methods: {
            getViewTimes(viewTimes) {
                this.majorViewTimes = viewTimes;
            },
//            initScrollWindow() {
//                return this.content || document.getElementsByClassName('comp').length
//            },
            /********************************************************************/
            changeSpecType: function (specType) {
...
//                    if (this.$refs.content.scrollTop - thead.offsetTop > -60 / 64 * parseInt(document.documentElement.style.fontSize)) {
...
//                        }
        ...
        mounted() {
            ...
            if (!this.$route.query.catId) {
                ...
                if (this.specType == 11) {
                    this.secondCatName = '历史录取'
                    this.title = '详情'
                }
                if (this.specType == 0) {
                    this.secondCatName = '专业详情'
                    this.title = this.$route.query.name
                }

                /**************此处添加新的面包屑文字模板************************/
                if (this.specType == 'schoolDetail') {
                    this.secondCatName = '院系介绍'
                    this.title = this.$route.query.name
                }
                /**************************************************************/
            ...
            this.$http.get(state.host + state.baseUrl + '/infoContent/getInfoContentDetail?infoId=' + this.infoId + userId + '&sCode=' + state.scode + '&parentId=' + this.catId)
                .then(res => {
                    console.log('请求getInfoContentDetail成功')
...
//                    this.content = data.content
                    ...
                    console.log(this.title, this.specType);
...
//                            alert('分享链接为：'+urlAddToken(window.location.href));
                            ...
                            wx.ready(function () {
                                wx.onMenuShareTimeline({
                                    title: state.allSchoolInfo.name + '-' + (this.tid ? '专业详情' : this.sId ? '院系详情' : '文章详情'), // 分享标题
...
//                            imgUrl: state.imgHost + iconUrl, // 分享图标
                                ...
                                wx.onMenuShareAppMessage({
                                    title: state.allSchoolInfo.name + '-' + (this.tid ? '专业详情' : this.sId ? '院系详情' : '文章详情'), // 分享标题
                                    desc: '点击查看' + (this.tid ? '专业详情' : this.sId ? '院系详情' : '文章详情'), // 分享描述
        ...
        created() {
            ...
            this.$http.get(state.host + state.baseUrl + '/index/getBaseInfo?sCode=' + state.scode + userId + login)
                .then(res => {
                    ...
                    document.title = data.school.name + '-' + (this.tid ? '专业详情' : this.sId ? '院系详情' : '文章详情')
