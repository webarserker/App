<template>
    <div class="schListHotContainer">

    <div class="schListHot">
        <div class="schBanner">
            <div class="swiper-container schBanner-swiper">
                <div class="swiper-wrapper">
                    <div class="swiper-slide" v-for="(obj,idx) of banner"
                         :style="{'background-image':'url(' + $store.state.imgHost+obj.backgroundImgUrl+')','background-size': 'cover','background-repeat':'no-repeat','background-position':'center center'}"
                         @click="gotoSchool(obj.code)">
                        <p class="desc" v-text="obj.name"></p>
                    </div>
                </div>
                <div class="pagination">
                    <div class="swiper-pagination pList"></div>
                </div>
            </div>
        </div>
        <div class="link" @click="$router.push('/schList/all/3')">即将上线高校<span class="icon-link">></span></div>
        <div>
            <!--高考招录-->
            <div class="listSet" v-if="normalSchList.length!=0">
                                ...
                                <div>
                                    <div class="schName">
                                        ...
                                        <div class="attribute" v-if="obj.recruitTypeList.length == 2">
                                            <span class="attr1" :class="{'gray': obj.recruitTypeList[0].status=='10'}">高招</span>
                                            <span class="attr2" :class="{'gray': obj.recruitTypeList[1].status=='10'}">研招</span>
                                        </div>
                                    </div>
                                    <div class="col22">
                                        ...
                                        <div class="left">
                                            ...
                                            <div class="schTyp">
                                                <span v-text="obj.type"></span> <span v-if="obj.type&&obj.property">|</span>
                                                <span v-text="obj.property"></span>
                                            </div>
                ...
                <div class="noMore" v-show="switch1 && schList.length==0">没有更多了...</div>
            </div>
            <!--研究生招录-->
            <div class="listSet" v-if="schList.length!=0">
                ...
                <div class="noMore" v-show="switch2">没有更多了...</div>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    export default {
        ...
        methods: {
            ...
            /*获取推广热门高校数据列表*/
            getExpandHotSchList: function () {
                var state = this.$store.state
                var url = state.host + state.baseUrl + '/school/findExpandHotSchoolList'
                setTimeout(() => {
                    this.$http.get(url, {
                        emulateJSON: true
                    })
            ...
            /*获取普通热门高校*/
            getNormalHotSchList: function () {
                var state = this.$store.state
                var url1 = state.host + state.baseUrl + '/school/findNormalHotSchoolList?userId=' + localStorage.uid + '&isPostGraduate=0'
                var url2 = state.host + state.baseUrl + '/school/findNormalHotSchoolList?userId=' + localStorage.uid + '&isPostGraduate=1'
                //高考招录
                setTimeout(() => {
                    this.$http.get(url1, {
                        emulateJSON: true
...
//                })
        ...
        created() {
            ...
            //获取推广热门高校
            this.getExpandHotSchList()
            //获取普通热门高校
            this.getNormalHotSchList()
            //tab切换时，筛选条件重置为空
