...
exports.default = {
    ...
    watch: {
        '$route': function $route(to, from) {
            ...
            this.$http.get(this.$store.state.host + '/front/template/getParentInfoCat?catId=' + this.catId).then(function (res) {
                res = res.body;
                console.log(res);
                sessionStorage.setItem('replace3catId', res.data);
                _this4.$router.push('/articleList?catId=' + res.data);
            });
            //                alert('分享link为：'+
    ...
    created: function created() {
        ...
        this.$http.get(this.$store.state.host + '/front/template/getParentInfoCat?catId=' + this.catId).then(function (res) {
            ...
            console.log(res);
            sessionStorage.setItem('replace3catId', res.data);
            _this6.$router.push('/articleList?catId=' + res.data);
        });
        ...
        this.$http.get(state.host + state.baseUrl + '/infoCat/getInfoCatlist?sCode=' + state.scode + '&catId=' + this.catId).then(function (res) {
            ...
            _this6.$store.commit('CHANGE_LOADINGFLAG', false);
            var secondCatId = res.data.data.secondCatId;
            _this6.CHANGE_SECONDNAVDATA(res.data.data.menu);
            for (var i = 0; i < _this6.secondNavData.length; i++) {
                if (secondCatId == _this6.secondNavData[i].catId) {
                    _this6.activeIndex = i;
                    break;
                }
            }
            i = i >= _this6.secondNavData.length ? 0 : i;
            _this6.endFlag = false;
            _this6.curPage = 1;
            var articleListCatId = '';
            var data = res.data.data.menu;
            if (data[i].children.length > 0) {
            ...
            _this6.articleListCatId = articleListCatId;

            sessionStorage.setItem('threeCatName', data[i].children.length > 0 ? data[i].children[0].name : '');

            _this6.CHANGE_SUBNAVDATA(data[i].children);
            if (_this6.$children[0].$refs.sub_navs_swiper_wrap) {
                _this6.$children[0].$refs.sub_navs_swiper_wrap.scrollLeft = 0;
                _this6.$children[0].tapIndex = 0;
                var state = _this6.$store.state;
                _this6.$http.get(state.host + state.baseUrl + '/infoContent/getInfoContentlist?curPage=' + _this6.curPage + '&pageSize=20&catId=' + articleListCatId).then(function (res) {
                    _this6.CHANGE_SUBCONDATA(res.data.data);
...
//# sourceMappingURL=0.7dd19be8677e7de8fbe2.hot-update.js.map