...
router.get('/auth', function(req, res){
    getOpenidUidOrRedirect(req, res, req.url, function(err, openid, uid){
        ...
        console.log('function auth>>>'+from);
        // 跳回来源页
        if(from != null && from == 'wenda') {
            // if(req.query.redirect) {
            //     res.redirect(config.wenda.host + ':' + config.wenda.port + req.query.redirect);
            // }else {
                console.log('wenda redirect>>>'+req.query.redirect);
                res.redirect(config.wenda.host + ':' + config.wenda.port + config.wenda.index);
            // }
        }else if(from != null && from == 'admin'){
            res.redirect(config.admin.host + ':' + config.admin.port + config.admin.index);
        }else {
...
router.get('/index', function(req, res){
    var token = req.query.spreadToken;
    ...
    waterfall([
        ...
        //系统来源
        var from = req.query.from;
        console.log('function index>>>'+from);
        console.log('function index userid>>>'+uid);
        // 跳回来源页
        if(from != null && from == 'wenda') {
            // if(req.query.redirect) {
                // res.redirect(config.wenda.host + ':' + config.wenda.port + req.query.redirect);
            // }else {
                console.log('wenda redirect 2>>>'+req.query.redirect);
                res.redirect(config.wenda.host + ':' + config.wenda.port + config.wenda.index + '#/?uid=' + uid);
            // }
        }else if(from != null && from == 'admin'){
            ...
            if(scode) {
                if(redirectUrl){
                    ...
                    //     token = 'NDUz';
                    // }
                    res.redirect('/build/index.html?scode=' + scode + '#/?uid='+uid+(token?'&spreadToken='+token:''));
                }
            }
            else{
                res.redirect('/build/index.html#/schList/hot?uid='+uid);
