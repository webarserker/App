<template>
  <div class="articleList">
    <SlideMenu :parentCatName="parentCatName" :pShowMenu="false">
      <span>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      this.$http.get(this.$store.state.host + '/front/findArticlesList' + '?parentCatId=' + this.$route.query.parentId + '&catId=' + this.$route.query.catId + '&curPage=' + this.curPage + '&pageSize=' + this.pageSize+'&userId='+localStorage.fuserid).then(res => {
        res = res.body;
        if (res.code == '000000') {
          let data = res.data;
          this.parentCatName = data.cat.parentCatName ? data.cat.parentCatName : data.cat.name;
          this.infoCatName = data.cat.name;


          this.newsList = data.page.data;
          if (this.newsList.length == 0) {
            this.hasList = false
          }
          if (data.page.data.length < this.pageSize) {
            this.noData = true;
          }
        }
...
//          res = res.body;
          ...
          this.$router.push({path: `/articleDetail?parentId=${this.$route.query.parentId}&catId=${this.$route.query.catId}&articleId=${id}`})
//        })
      },
      loadMore() {
        if (!this.noData) {
          ...
          this.curPage++;
          this.$http.get(this.$store.state.host + '/front/findArticlesByCatId' + '?catId=' + this.$route.query.catId + '&curPage=' + this.curPage + '&pageSize=' + this.pageSize+'&userId='+localStorage.fuserid).then(res => {
            res = res.body;
