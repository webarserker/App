...
//    	$(ul).show();
		...
		var _deferredObj = $.get(url, params, function(data) {
			for (var i = 0; i < data.length; i++) {
                ...
                if (idName != 'this') {
                    ...
                    jsonStr = jsonStr.substr(0, arrStr.length - 1);
                    jsonStr += "]";
                }
                json['obj'] = jsonStr;
				array[i] = json;
    ...
    listenUl : function() {
    	...
    	$('.inputSelect').on('keydown', function(e) {
    		var _this = this;
    		var ul = $(this).parent().children(".selectUl");
    	...
    	//下拉框输入监听
        $('.inputSelect').on('input',inputSelectUtil.selectChange);
        
        //下拉框失去焦点监听
        $('.inputSelect').on('blur',function(e){
    		var _this = this;
