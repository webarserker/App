...
var v5templateHome = require('./v5templateHome');
var fs = require('fs');
...
function loginPage(req,res,next){
    ...
    v5templateHomeUtil.findSchool(sCode,function(err,data){
        ...
        retJson.urlForgetPassword= base_sever_url + "/v5users/v5forgetPassword";
        // 免费注册
        retJson.urlRegisterPage= base_sever_url + "/v5users/v5registerPage";
        // 首页
        retJson.urlHomePage = base_sever_url+ "/v5templateHome/v5templateHomePage";
        retJson.themeColor = retJson.schoolInfo.styleColor; // 将学校信息中的主题色提取至模型根部, 便于 jade 读取
        retJson.sidebarBgImg = retJson.schoolInfo.styleSidebarBg;  // 将二级侧边栏中的信息提取至模型根部,便于jade读取
        retJson.navBgImg = retJson.schoolInfo.styleNavBg;          // 将首页导航栏中的背景主题色提取至模型根部,便于jade读取
        //console.log(JSON.stringify(retJson));
...
function onlineMessagePage(req,res,next){
    ...
    async.series([
        function(callback) {
            // 获取学校基本配置信息 **** 0
            v5templateHomeUtil.findSchool(sCode,function(err,data){callback(err,data)});
        },
        function(callback){
            v5templateHomeUtil.getNoticeNum(sCode,userToken,function(err,data){return callback(err,data)});
        },//****  消息列表总数接口 **** 1
...
function saveQuestion_json(req,res,next){
    ...
    superagent.get(url)
        ...
        .end(function (err, res2) {
            ...
            var retJson = JSON.parse(res2.text);
            // 重新刷新一次本页面
            retJson.urlOnlineMessage = base_sever_url + "/v5users/v5onlineMessage?"+"schoolCode="+schoolCode;
            res.set('Content-Type', 'application/json');
            res.json(retJson);
...
function messageListPage(req,res,next){
    ...
    async.series([
        ...
        function(callback) {
            // 获取消息列表 **** 1
            v5templateHomeUtil.getNoticeList(sCode,userToken,function(err,data){callback(err,data)});
        },
        function(callback){
            v5templateHomeUtil.getNoticeNum(sCode,userToken,function(err,data){return callback(err,data)});
        },//****  消息列表总数接口 **** 2
