<template>
    <div class="contentList">
      <Notice></Notice>
      <Tab entryName="答疑审核"></Tab>
      <div class="none" v-show="contentList2.length==0 && !unLoad">
        <div>
      ...
      <div class="wrapper" @scroll="scrollUl($event,3)">
        <ul class="scroll-ul" v-show="contentList2.length>0">
          <li v-for="(item,index) in contentList2" @click="item.isRead=true;newsDetail(item.id)">
            <div class="title">
              <p>
                {{item.question}}
...
<script>
  ...
  export default {
    ...
    created(){
      ...
      this.$http.get(this.$store.state.host + '/answer/findToAuditAnswerList' + '?userToken=' + localStorage.zb_userToken
        + '&sCode=' + localStorage.zb_sCode + '&curPage=' + this.curPage + '&pageSize=' + this.pageSize).then((res) => {
        res = res.body;
        ...
        if (res.code = '000000') {
          this.contentList2 = res.data.data;
...
//        this.$store.commit('SET_LOADING_STATU', true);
        this.$http.get(this.$store.state.host + '/sysUser/saveContentReadRecord' + '?userToken=' + localStorage.zb_userToken + '&sCode=' + localStorage.zb_sCode
          + '&userType=20' + '&contentId=' + id + '&contentType=15').then(res => {
          res = res.body;
      ...
      loadMore(){
        ...
        if (!this.hasNoMore) {
          ...
          this.$http.get(this.$store.state.host + '/answer/findToAuditAnswerList' + '?userToken=' + localStorage.zb_userToken
            ...
            if (res.code = '000000') {
              this.contentList2 = this.contentList2.concat(res.data.data);
              if (res.data.data.length < this.pageSize) {
                this.hasNoMore = true;
              }
            }else{
